defmodule OnetimeTest do
  ...
  test "pop with secs", context do
    Onetime.register(context[:pid], :a, :A)
    Onetime.register(context[:pid], :b, :B, DateTime.now() |> DateTime.shift(secs: -1001))
    Onetime.register(context[:pid], :c, :C, DateTime.now() |> DateTime.shift(secs: -1000))
  ...
  test "get_all with secs", context do
    Onetime.register(context[:pid], :a, :A)
    Onetime.register(context[:pid], :b, :B, DateTime.now() |> DateTime.shift(secs: -1000))
    Onetime.register(context[:pid], :c, :C, DateTime.now() |> DateTime.shift(secs: -999))
    assert %{a: :A, c: :C} == Onetime.get_all(context[:pid], 1000)
  end
